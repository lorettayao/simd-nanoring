...
| 4 | Every consumer reads all messages | "75% of cache loads are wasted per thread." | "Yes — the broadcast pattern sacrifices per-thread efficiency for simplicity. In production I'd either: (a) push sharding into the decoder so each journal only carries one shard's traffic, or (b) use per-shard queues instead of broadcast." |
